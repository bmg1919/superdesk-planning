...
def init_app(app):
    ...
    PlanningExportTemplatesResource(
        PlanningExportTemplatesResource.endpoint_name,
        app=app,
    ...
    superdesk.register_default_user_preference(
        "planning:agenda",
        {"type": "dict", "agenda": {}, "default": None},
        label=lazy_gettext("Default Agenda"),
        category=lazy_gettext("Planning"),
    )

    superdesk.register_default_user_preference(
        "planning:events_planning_filter",
        {"type": "dict", "filter": {}, "default": None},
    ...
    if app.config.get("PLANNING_EXPIRY_MINUTES", 0) != 0 and not app.config.get("CELERY_BEAT_SCHEDULE").get(
        "planning:expiry"
    ):
    ...
    if app.config.get("PLANNING_DELETE_SPIKED_MINUTES", 0) != 0 and not app.config.get("CELERY_BEAT_SCHEDULE").get(
        "planning:delete"
    ):
